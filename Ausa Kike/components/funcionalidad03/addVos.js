...
function f03escribirAusa(id, url, idTarea, nombre) {
    //Notificaciones
    var notificationElement = $("#notification");
    notificationElement.kendoNotification();
    var notificationWidget = notificationElement.data("kendoNotification");
    ...
    $.ajax({
        ...
        data: {
            ...
            url: url,
            tipo: 1,
            subPath: idTarea //Se cambió a numero de orden
        },
        beforeSend: function () {
            // setting a timeout
            kendo.ui.progress($("#f03div" + id), true);
        },
        error: function (xhr) { // if error occured
            notificationWidget.show("El servicio no está disponible", "error");
        },
        complete: function () {
            kendo.ui.progress($("#f03div" + id), false);
        }
    }).done(function (datos) {
        if (parseInt(datos) == 0) {
            $('#f03viewAudios').data('kendoListView').dataSource.read();
...
function f03eliminarEverlive(idAudioBackend) {
    //Para borrar del backend service
    var el = new Everlive('80i2xn90wysdmolz');
    var data = el.data('Archivos');
    data.destroySingle({
...
function f03reUpload(Id) {
    var url = document.getElementById("archivo" + Id).value;
    var nombre = url.replace(/^.*[\\\/]/, '');
    f03escribirAusa(Id, url, $('#txtid').val(), nombre); //id del bs, url del bs    
}